...
##
## A class for orthogonal subspace projections
##

## Initialize data structure of class mvar.space
##   $M     ... optional data matrix (row vectors in original coordinates)
##   $basis ... basis spanning subspace (as specified by user, column vectors)
##   $P     ... orthonormal basis of subspace (column vectors)
##   $Q     ... orthonormal PCA basis of complement space (column vectors)
mvar.space <- function (M, basis, dim=ncol(M), ...) {
  if (missing(M)) {
    if (missing(dim)) stop("either M= or dim= must be specified")
    M <- diag(dim)
  }
  if (!is.matrix(M)) stop("M= must be a matrix")
  n <- dim # dimensionality of data space
  if (ncol(M) != n) stop("dim= doesn't match dimensionality of M=")
  if (nrow(M) < n) stop("M= must provide at least as many data points as there are dimensions")
  P <- matrix(0, nrow=n, ncol=0) # target sub-space has 0 dimensions so far
  Q <- prcomp(M, center=TRUE, scale=FALSE)$rotation
  res <- list(M=M, basis=P, P=P, Q=Q)
  class(res) <- c("mvar.space", "list")
  if (!missing(basis)) mvar.add(res, basis, ...) else res
}

print.mvar.space <- function(x, ...) {
  stopifnot(inherits(x, "mvar.space"))
  k <- nrow(x$M)
  n <- ncol(x$M)
  d1 <- ncol(x$P)
  cat(sprintf("mvar.space object representing projection of %d x %d data matrix into %d-dimensional subspace\n", k, n, d1))
}
...
mvar.R2 <- function (x, M=x$M, dim=seq_len(ncol(x$P))) {
  stopifnot(inherits(x, "mvar.space"))
  ...
  Mproj <- mvar.projection(x, space="both", M=M)
...
## add one or more basis vectors to subspace
...
mvar.add <- function (x, basis, space=c("orig", "both", "complement"), names=colnames(basis), normalize=FALSE,  tol=1e-7) {
  if (!inherits(x, "mvar.space")) stop("first argument must be mvar.space object")
  space <- match.arg(space)
  if (is.vector(basis)) basis <- colVector(basis) # propagate vector to 1-column matrix
  n <- ncol(x$M)    # dimensionality of original space
  d1 <- ncol(x$P)   # dimensionality of current subspace
  ...
  if (space == "complement") {
    ...
    basis <- x$Q %*% basis  # transform into original space
  } else if (space == "both") {
    if (nb != n) stop(sprintf("basis vectors of length %d don't match %d-dimensional orthonormal space", nb, n))
    basis <- cbind(x$P, x$Q) %*% basis # transform into original space
  } else {
  ...
  Pnew <- orthogonalize(cbind(x$P, basis)) # extend projection subspace
  if (ncol(Pnew) < d1 + d2) stop("basis vectors do not form a linearly independent extension of the current subspace")
  if (d1 > 0) {
    if (max(abs(Pnew[, seq_len(d1), drop=FALSE] - x$P)) > tol) stop("internal error -- QR decomposition does not preserve existing orthonormal basis")
  }
  if (!is.null(names)) {
    if (length(names) != d2) stop("length of names= doesn't match number of basis vectors")
    colnames(basis) <- names
  }
  x$basis <- cbind(x$basis, basis) # extend list of original basis vectors
  colnames(Pnew) <- colnames(x$basis)
  x$P <- Pnew # orthonormal basis of new subspace
  
  Mcomp <- x$M - x$M %*% tcrossprod(Pnew) # M * (I - P P') = projection of row matrix M into new complement space
  ...
  if (max(abs(crossprod(PQnew) - diag(n))) > tol) stop("internal error -- basis of subspace + complement is not orthogonal")
  x$Q <- Qnew
  
  rownames(x$P) <- rownames(x$Q) <- rownames(x$basis) <- colnames(x$M)
  x
}


## apply rotation to selected subspace dimensions
##  - any subset of the subspace dimensions can be selected with dims=; since
##    all are orthogonal, rotations on subsets are completely independent
## with type= selecting one of the following rotation types:
## "pca":
##  - performs a PCA on M= in the subspace and rotates the dimensions to principal components
## "swap":
##  - reorder dimensions according to the permutation in perm= (default: reverse order)
## "flip":
##  - reverse sign of selected dimensions
## "match":
##  - reorder and flip dimensions to give best match to those in basis=
## TODO
##  * improve "match" mode
...
##    - only orthogonal rotation will be allowed, of course
mvar.rotation <- function (x, type=c("pca", "swap", "flip", "match"), dims=NULL, 
                           M=x$M, perm=rev(seq_len(d)), basis=NULL) {
  type <- match.arg(type)
  k <- ncol(x$P) # dimensionality of target space
  if (is.null(dims)) dims <- seq_len(k)
  stopifnot(length(dims) >= 1, all(1 <= dims & dims <= k), !any(duplicated(dims)))
  P1 <- x$P[, dims, drop=FALSE] # subspace for the rotation (of dimensionality d)
  d <- length(dims)
  if (type == "pca") {
    M1 <- x$M %*% P1  # projection of M into rotation subspace
    ...
    stopifnot(all.equal(sort(perm), seq_len(d))) # check that perm= is a suitable permutation
    ...
    P1.new <- matrix(nrow=nrow(P1), ncol=d)
    for (i in seq_len(d)) {
...
mvar.basis <- function (x, space=c("space", "both", "complement", "axes"), dim=NULL) {
  stopifnot(inherits(x, "mvar.space"))
  space <- match.arg(space)
  res <- switch(space,
    "both" = cbind(x$P, x$Q),
...
mvar.projection <- function (x, space=c("space", "both", "complement", "axes"), dim=NULL, M=x$M, byrow=TRUE) {
  stopifnot(inherits(x, "mvar.space"))
  space <- match.arg(space)
  n <- if (byrow) ncol(M) else nrow(M)
  if (space == "axes") stop("what='axes' is not supported yet")
  P <- mvar.basis(x, space, dim=dim) # orthonormal basis for the projection
  if (nrow(P) != n) stop("dimensionalty of basis vectors doesn't match data matrix")
  if (byrow) M %*% P else crossprod(P, M)
}

## apply operation in subspace coordinates and re-transform vector into original space
##   - M = data matrix of row or column vectors to be operated on
##   - P = orthornormal basis of the subspace (column vectors)
...
##   - byrow.out = whether result is a row or column matrix
##   - check = whether to check that basis P is orthonormal
mvar.subspace.apply <- function (M, P, FUN, ..., byrow.in=TRUE, byrow.out=TRUE, check=TRUE, tol=1e-7) {
  n <- if (byrow.in) ncol(M) else nrow(M)
  if (nrow(P) != n) stop("dimensionality of basis vectors doesn't match data matrix")
  d <- ncol(P)
  if (check && max(abs(crossprod(P) - diag(d))) > tol) stop("subspace basis is not orthonormal") 
  M1 <- if (byrow.in) M %*% P else crossprod(P, M)
  res1 <- FUN(M1, ...)
  n1 <- if (byrow.out) ncol(res1) else nrow(res1)
  if (n1 != d) stop("dimensionality of result doesn't match subspace -- did you forget to set byrow.out?")
  if (byrow.out) tcrossprod(res1, P) else P %*% res1
}

## identify LDA dimensions in full space or specified subspace
##   - M = data matrix of row vectors
##   - categories = vector of categories to be separated by LDA
##   - P = orthonormal basis of subspace (optional)
##   - idx = perform LDA on subset of M
mvar.discriminant <- function (M, categories, P=diag(ncol(M)), idx=NULL) {
  ...
  lda.dims <- function (...) lda(...)$scaling
  res <- mvar.subspace.apply(M, P, lda.dims, categories, byrow.out=FALSE)
...
## determine obliqueness between two subspaces P and Q -- THIS MEASURE IS DEPRECATED
##  - obliqueness is a measure of whether P contains "information" that is not also captured by Q
##  - it is defined as the largest angle between a vector in P and its projection into Q
##  - NB: obliqueness is not symmetric: if dim P > dim Q, obliqueness of P over Q is always 90 deg
##  - NB: obliqueness is not a measure of how much information P adds to a combined space
##  - P = (orthonormal) basis of P
##  - Q = (orthonormal) basis of Q
##  - byrow = if TRUE, bases of P and Q are specified as row vectors
##  - orthogonalize = if TRUE, P and Q need not be orthonormal
mvar.obliqueness <- function (P, Q, byrow=FALSE, orthogonalize=FALSE, tol=1e-7) {
  if (byrow) {
  	P <- t(P)
  	Q <- t(Q)
  }
  ...
  if (orthogonalize) {
  	res <- qr(P)
  	if (res$rank != nP) stop("P is not a basis (doesn't have full rank)")
  	P <- qr.Q(res)
  	res <- qr(Q)
  	if (res$rank != nQ) stop("Q is not a basis (doesn't have full rank)")
  	Q <- qr.Q(res)
  } else {
    if (max(abs(crossprod(P) - diag(nP))) > tol) stop("basis of subspace P is not orthonormal")
    if (max(abs(crossprod(Q) - diag(nQ))) > tol) stop("basis of subspace Q is not orthonormal")
  }
  ## - obliqueness = largest angle phi between a unit vector x in P and its projection into Q
  ## - cos(phi) = length of projection Q'x
  ## - for orthonormal P we can write x = Pa for unit vector a with dim(P) dimensions
  ## - Q'x = Q'Pa = Ra with R = Q'P (the projection of the basis vectors P into Q)
  ## - given SVD  R = U S V'  we have |Q'x| = |USV'x| = |SV'x| with V' an isometry of the unit sphere
  ## - hence min |Q'x| = min |Sy| with |y| = 1 (y = V'x) --> smallest singular value in S
  ## - if dim(P) > dim(Q), R has an "implicit" singular value of 0 (Q'P linearly dependent), so we need to handle this case separately
  if (nP > nQ) return(90)
  R <- crossprod(Q, P)
  res <- svd(R)
  len.Qt.x <- min(res$d)
  if (len.Qt.x < tol) len.Qt.x <- 0   # probably rounding errors
  if (len.Qt.x > 1-tol) len.Qt.x <- 1 
  180 * acos(len.Qt.x) / pi
}
...
##  - if orthogonalize=FALSE, A and B must already be column-orthogonal
...
##  - but better as method of mvar space object so extra dims default to PCA rather than random
...
## project into specified (orthogonal) dimensions, then apply optional callback to complement
##   - Q contains orthonormal basis of target subspace as column vectors
##   - callback function called with projection of M into complement space + extra arguments; must return complete orthonormal basis of this subspace (i.e. rotation of coordinates in the complement)
##   - returns projection of M into computed coordinates of subspace + complement
##   - the complete projection matrix, i.e. an orthonormal basis of the full space, is attached as attr "Q"
apply.projection <- function (M, Q, orthogonalize=FALSE, complement.projection=NULL, ...) {
  nR <- nrow(Q); nC <- ncol(Q) # nR = dim of original space, nC = dim of subspace
  stopifnot(nC < nR)
  stopifnot(nR == ncol(M))
  if (orthogonalize) {
    Q <- qr.Q(qr(Q))
    colnames(Q) <- paste("P", 1:ncol(Q), sep="")
  } else {
    stopifnot(all(abs(crossprod(Q) - diag(1, nC, nC)) < 1e-12)) # ensure Q is indeed orthonormal
  }
  P <- tcrossprod(Q) # subspace projection in natural coordinates
  Pc <- diag(1, nR, nR) - P # orthogonal projection into complement
  Qc <- svd(Pc, nu=(nR-nC), nv=0)$u # orthonormal basis of the complement
  if (!is.null(complement.projection)) {
    Mc <- M %*% Qc # projection of data points into complement, in Qc-coordinates
    rotation <- complement.projection(Mc, ...)
    Qc <- Qc %*% rotation    
  }
  Q <- cbind(Q, Qc)
  rownames(Q) <- colnames(M)
  Mproj <- M %*% Q
  attr(Mproj, "Q") <- Q
  Mproj
}

## extend projection Q with PCA on orthogonal complement of span(Q)
complement.pca <- function (M, Q, ...) {
  apply.projection(M, Q, complement.projection=(function (Mc, ...) prcomp(Mc)$rotation), ...)
}


##
## visually identify linear discriminant (using supervised LDA in low-dimensional subspace)
##
##   - Q transforms into desired subspace (need not be orthonormal), specify Q=diag(n) to work in M space
find.discriminant <- function (M, Q, categories, idx=1:nrow(M)) {
  stopifnot(nrow(M) == length(categories))
  if (!missing(idx)) categories <- factor(as.character(categories)[idx])
  ldares <- lda(M[idx,,drop=FALSE] %*% Q, categories)
  discriminant <- Q %*% ldares$scaling # columns of Q transform into supervised LDA dimensions
  discriminant
}
...
ggbar.weights <- function (basis, dim=1:ncol(basis), idx=NULL, names=NA, feature.names=NA, ylab="normalized feature weights", main=NULL, bw=FALSE, base_size=12, ylim=NULL) {
  n <- nrow(basis)
  k <- length(dim)
  stopifnot(all(dim >= 1 & dim <= ncol(basis)))
  if (isNA(names)) names <- paste("dim", dim)
  ...
  if (bw) {
    ...
    bp <- bp + scale_fill_gradient2(low="#CC0000", mid="white", high="#00AA00")
  }
...
##  - ylim = display range on y-axis (NULL or two-element vector)
## returns a ggplot object that has to be printed to display the graph
ggbox.features <- function (M, Meta, what=c("features", "weighted", "contribution"), weights=rep(1, ncol(M)), group=NULL, group.palette=NULL, subset=NULL, select=NULL, feature.names=NULL, id.var="filename", main=NULL, nrow=2, bw=FALSE, base_size=12, diamond.size=2.5, group.labels=TRUE, ylim=NULL) {
  n <- nrow(M)
  k <- ncol(M)
...
mvar.pairs <- function (M, dims=NULL, Meta=NULL, select=NULL, pch=NULL, col=NULL, pch.vals=1:10, pch.cols=NULL, col.vals=corpora.palette("simple"), cex=1, legend.cex=1.4*cex, randomize=TRUE, gap=.5, oma=c(2,2,2,2), iso=FALSE, compact=FALSE, ...) {
...
##
mvar.clust <- function (clusters, labels=clusters$labels, pch=NULL, col=NULL, pch.vals=c(16,17,18,15,0:6), pch.cols=NULL, col.vals=corpora.palette("simple"), legend=TRUE, cex=1, legend.cex=cex, spread=.1, period=1, xlab="", ...) {
...
make.movie <- function (basename, axis=c(0,1,0), repeats=1, rpm=10, fps=24, theta=0, phi=20, zoom=.6, axis2=NULL, keep.frames=FALSE, out.dir=".", tmp.dir=tempdir(check=TRUE), dry.run=FALSE) {
  ...
  if (dry.run) {
    ...
    movie3d(camera.fnc, n.sec, movie=basename, dir=tmp.dir, fps=fps, clean=!keep.frames, convert="ffmpeg -r %s -i %s%%03d.png -s 1024,768 -vcodec libx264 -pix_fmt yuv420p -preset slow -profile:v main -crf 5 %s.mp4")
